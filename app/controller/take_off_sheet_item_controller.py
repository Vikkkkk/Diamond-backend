...
from repositories.take_off_sheet_repositories import (
    delete_opening,
    clone_sheet_area_item,
    is_opening_number_exists,
)
...
from schemas.take_off_sheet_item_schema import (
    TakeOffSheetItem,
    TakeOffSheetCloneRequest,
)
...
async def get_adon_opening_fileds(db: Session):
    ...
    **Returns:**
    ...
    try:
        ...
        logger.exception(
            f"get_adon_opening_fileds:: An unexpected error occurred: {error}"
        )
        raise error
...
async def get_take_off_sheet_item_details(item_id: str, db: Session):
    ...
    **Returns:**
    ...
    try:
        ...
        logger.exception(
            f"get_take_off_sheet_item_details:: An unexpected error occurred: {error}"
        )
        raise error
    ...
    **Return:**
    ...
    try:
        item_data = (
            db.query(ProjectTakeOffSheetSectionAreaItems)
            .filter(
                ProjectTakeOffSheetSectionAreaItems.project_take_off_sheet_section_area_id
                == take_off_sheet_area_id,
                ProjectTakeOffSheetSectionAreaItems.is_deleted == False,
            )
        ...
        logger.exception(
            f"get_take_off_sheet_area_items:: An unexpected error occurred: {error}"
        )
        raise error
        ...
        if len(take_off_sheet_data) > 0:
            ...
            if keyword:
                openings = openings.filter(
                    ProjectTakeOffSheetSectionAreaItems.opening_number.ilike(
                        f"%{keyword}%"
                    )
                )
...
async def add_take_off_sheet_item(
    take_off_sheet_item_req_data: TakeOffSheetItem, current_member: Members, db: Session
):
    """**Summary:**
    ...
    **Return:**
    ...
    try:
        ...
        # Begin a transaction
        with db.begin():
            take_off_sheet_item_req_data = take_off_sheet_item_req_data.model_dump(
                exclude_unset=True
            )
            take_off_sheet_item_req_data["created_by"] = current_member.id
            project_take_off_sheet_id = take_off_sheet_item_req_data[
                "project_take_off_sheet_id"
            ]
            opening_number = take_off_sheet_item_req_data["opening_number"]
            project_takeoffsheet_section_area_opening_exist = (
                await is_opening_number_exists(
                    project_take_off_sheet_id, opening_number, db
                )
            )
            ...
            if project_takeoffsheet_section_area_opening_exist:
                return JSONResponse(
                    content={
                        "message": "Opening Number already exists in the current Section"
                    },
                    status_code=400,
                )
            ...
            (
                take_off_sheet_item_req_data["door_width"],
                take_off_sheet_item_req_data["door_height"],
            ) = [",".join(["_"] * door_count)] * 2

            is_batch_insert = take_off_sheet_item_req_data.get("is_batch_insert", False)
            batch_insert_quantity = take_off_sheet_item_req_data.get(
                "batch_insert_quantity", 0
            )
            del take_off_sheet_item_req_data["is_batch_insert"]
            del take_off_sheet_item_req_data["batch_insert_quantity"]

            if is_batch_insert:

                opening_list = []
                stripped_opening_number = opening_number.strip()
                trailing_number_match = re.search(r"(\d+)$", stripped_opening_number)
                prefix = None
                starting_number = None
                number_width = None

                if trailing_number_match:
                    matched_number = trailing_number_match.group(1)
                    prefix = stripped_opening_number[: -len(matched_number)]
                    starting_number = int(matched_number)
                    number_width = len(matched_number)

                generated_opening_numbers = []
                for i in range(batch_insert_quantity):
                    if i == 0:
                        new_opening_number = opening_number
                    elif trailing_number_match:
                        incremented_number = str(starting_number + i).zfill(
                            number_width
                        )

                        new_opening_number = f"{prefix}{incremented_number}"
                    else:

                        new_opening_number = f"{stripped_opening_number}_{i}"
                    generated_opening_numbers.append(new_opening_number)

                existing_openings_query = (
                    db.query(ProjectTakeOffSheetSectionAreaItems.opening_number)
                    .filter(
                        ProjectTakeOffSheetSectionAreaItems.project_take_off_sheet_id
                        == project_take_off_sheet_id,
                        ProjectTakeOffSheetSectionAreaItems.is_deleted == False,
                        ProjectTakeOffSheetSectionAreaItems.opening_number.in_(
                            generated_opening_numbers
                        ),
                    )
                    .all()
                )
                existing_openings = [row[0] for row in existing_openings_query]

                if existing_openings:
                    return JSONResponse(
                        content={
                            "message": f"Opening numbers already exist: {', '.join(existing_openings)}"
                        },
                        status_code=400,
                    )
                for opening in generated_opening_numbers:
                    item_data = ProjectTakeOffSheetSectionAreaItems(
                        **{**take_off_sheet_item_req_data, "opening_number": opening}
                    )
                    opening_list.append(item_data)

                db.add_all(opening_list)
                db.flush()
                return {
                    "message": "Area items added successfully.",
                    "status": "success",
                }
            else:

                item_data = ProjectTakeOffSheetSectionAreaItems(
                    **take_off_sheet_item_req_data
                )
                db.add(item_data)
                db.flush()
                project_take_off_sheet_section_area_item_id = item_data.id
                return {
                    "id": project_take_off_sheet_section_area_item_id,
                    "message": "Area item added successfully.",
                    "status": "success",
                }
    except Exception as error:
        ...
        with db.begin():
            ...
            if project_takeoffsheet_item_exist:
                project_take_off_sheet_section_id = (
                    project_takeoffsheet_item_exist.project_take_off_sheet_section_id
                )
                ...
                await update_section_stats(
                    db,
                    project_take_off_sheet_section_id=project_take_off_sheet_section_id,
                )

                # update the sheet stats after deleting the section area and its associated openings
                await update_take_off_sheet_stats(
                    db,
                    project_take_off_sheet_id=project_takeoffsheet_item_exist.project_take_off_sheet_id,
                )
                ...
                project_take_off_data = (
                    ...
                    .filter(
                        ...
                        == project_takeoffsheet_item_exist.project_take_off_sheet_id
                    )
                    .first()
                )
                ...
                return JSONResponse(
                    content={"message": "Item not found"}, status_code=400
                )
    except Exception as error:
        # Handle other unexpected errors by logging the exception, rolling back changes, and re-raising the exception
        logger.exception(f"An unexpected error occurred: {error}")
    ...
    try:
        ...
        with db.begin():
            ...
            if item:
                ...
                if "opening_number" in take_off_sheet_item_req_data and str(
                    take_off_sheet_item_req_data["opening_number"]
                ) != str(item.opening_number):
                    # In case user is trying to update the opening number of the current item
                    opening_number = take_off_sheet_item_req_data["opening_number"]
                    project_takeoffsheet_section_area_opening_exist = (
                        await is_opening_number_exists(
                            item.project_take_off_sheet_id, opening_number, db
                        )
                    )
                    if project_takeoffsheet_section_area_opening_exist:
                ...
                if valid_opening_number:
                    ...
                    old_door_type, new_door_type = (
                        item.adon_fields["door_type"],
                        take_off_sheet_item_req_data["adon_fields"]["door_type"],
                    )
                    old_door_count, new_door_count = (
                        id_to_count_mapping[old_door_type],
                        id_to_count_mapping[new_door_type],
                    )
                    if new_door_count > old_door_count:
                        ...
                        (
                            take_off_sheet_item_req_data["door_width"],
                            take_off_sheet_item_req_data["door_height"],
                        ) = (item.door_width + postfix, item.door_height + postfix)
                        ...
                        if empty_slots >= diff:
                            ...
                            take_off_sheet_item_req_data["door_width"] = ",".join(
                                functools.reduce(
                                    lambda acc, cur: (
                                        acc + [cur]
                                        if cur != "_"
                                        or acc.count("_") < can_remain_empty_count
                                        else acc
                                    ),
                                    item.door_width.split(","),
                                    [],
                                )
                            )
                            take_off_sheet_item_req_data["door_height"] = ",".join(
                                functools.reduce(
                                    lambda acc, cur: (
                                        acc + [cur]
                                        if cur != "_"
                            ...
                            raise Exception(
                                "Unable to reduce door type!, remove "
                                + str(diff)
                                + " "
                                + ("door" if diff == 1 else "doors")
                                + " from the opening first!"
                            )
                    ...
                    if "quantity" in take_off_sheet_item_req_data:
                        ...
                        take_off_sheet_section_id = await update_area_item_stats(
                            db,
                            project_take_off_sheet_section_area_item_id=take_off_sheet_item_id,
                        )
                        ...
                        await update_take_off_sheet_stats(
                            db,
                            project_take_off_sheet_id=project_take_off_sheet_area_item_data.project_take_off_sheet_id,
                        )
                        ...
                        project_take_off_sheet_data = (
                            ...
                            .filter(
                                ProjectTakeOffSheets.id
                                == project_take_off_sheet_area_item_data.project_take_off_sheet_id
                            )
                            .first()
                        )
                    ...
                    return JSONResponse(
                        content={
                            "message": "Opening Number already exists in the current area"
                        },
                        status_code=400,
                    )
            else:
...
async def get_associate_item(
    db: Session, take_off_sheet_area_item_id: str, material_type: str
):
    """**Summary:**
        ...
        if material_type == "HARDWARE":
            for opening_schedule_info in opening_schedule_data:
                ...
                item_data = (
                    ...
                    .filter(
                        HardwareGroupMaterials.hardware_group_id
                        == opening_schedule_info.hardware_group_id,
                        ProjectMaterials.is_deleted == False,
                    )
                    .order_by(HardwareGroupMaterials.created_at.desc())
                    .all()
                )
                hw_group_data = db.query(HardwareGroups).get(
                    opening_schedule_info.hardware_group_id
                )
                ...
                for data in item_data:
                    ...
                    data_dict["hardware_group_id"] = (
                        opening_schedule_info.hardware_group_id
                    )
                    data_dict["quantity"] = matching_entries[0].quantity
                    data_dict["final_amount"] = matching_entries[0].final_amount
                    data_dict["final_base_amount"] = matching_entries[
                        0
                    ].final_base_amount
                    data_dict["final_sell_amount"] = matching_entries[
                        0
                    ].final_sell_amount
                    data_dict["final_extended_sell_amount"] = matching_entries[
                        0
                    ].final_extended_sell_amount
                    data_dict["manufacturer_code"] = data.material_manufacturer.code
                    data_dict["brand_code"] = (
                        data.material_brand.code if data.material_brand else None
                    )
            ...
            for data in opening_schedule_data:
                ...
                data_dict["file"] = (
                    {
                        ...
                        "file_path": get_aws_full_path(
                            data.opening_material.content_file_path
                        ),
                        "file_type": data.opening_material.content_file_type,
                    }
                    if data.opening_material.content_file_path is not None
                    else None
                )
                data_dict["description"] = data.opening_material.desc
                data_dict["series"] = data.opening_material.series
                data_dict["manufacturer_code"] = (
                    data.opening_material.material_manufacturer.code
                    if data.opening_material.has_pricebook
                    else None
                )
                data_dict["brand_code"] = (
                    data.opening_material.material_brand.code
                    if data.opening_material.material_brand
                    else None
                )
                ...
                data_dict["short_code"] = (
                    data.opening_material.short_code
                    if data.opening_material.short_code
                    else None
                )
                response.append(data_dict)
...
async def clone_opening(
    ...
    try:
        ...
        if not item_data:
            return JSONResponse(
                content={"message": "Invalid request."}, status_code=400
            )
        ...
        is_opening_exists = await is_opening_number_exists(
            item_data["project_take_off_sheet_id"], request_data["opening_number"], db
        )
        if is_opening_exists:
            return JSONResponse(
                content={"message": "Opening Number already exists."}, status_code=400
            )
        ...
        if (
            "project_take_off_sheet_section_id" in request_data
            and request_data["project_take_off_sheet_section_id"] is not None
        ):
            take_off_sheet_section_id = request_data[
                "project_take_off_sheet_section_id"
            ]
        else:
        ...
        if (
            "project_take_off_sheet_section_area_id" in request_data
            and request_data["project_take_off_sheet_section_area_id"] is not None
        ):
            take_off_sheet_section_area_id = request_data[
                "project_take_off_sheet_section_area_id"
            ]
        else:
            take_off_sheet_section_area_id = item_data[
                "project_take_off_sheet_section_area_id"
            ]
    ...
    **Returns:**
    ...
    try:
        ...
        if not project_take_off_sheet:
            return JSONResponse(
                content={
                    "message": "No take-off sheet found for the provided project ID."
                },
        ...
        item_data_query = (
            ...
            .join(Sections, ProjectTakeOffSheetSections.section_id == Sections.id)
            .join(
                ProjectTakeOffSheetSectionAreas,
                ProjectTakeOffSheetSectionAreaItems.project_take_off_sheet_section_area_id
                == ProjectTakeOffSheetSectionAreas.id,
            )
            .join(
                OpeningSchedules,
                OpeningSchedules.project_take_off_sheet_section_area_item_id
    ...
    try:
        project_material = (
            ...
            .filter(
                ...
                ProjectMaterials.id == project_material_id,
            )
            .first()
        )
